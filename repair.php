...
<body>
    <div class="div2">
        <ul>
            ...
            <li><a style="float: left;" onclick= "location.href = 'service.php';">Next</a></li>
          ...
          <div class="devrepair_container">
            <div class="row">
              <div class="devicerepair_container">
                ...
                <p>Please fill in this field to register as a repair customer.</p><br>
                <form action="" id="dev_date" method="POST">
                  <label>Repair ID must be all numbers:</label><br>
                  <input type="text" name="repid" placeholder="Enter Repair ID" required></input><br>
                ...
                <?php
                  ...
                  if (isset($_POST['submit'])) {
                      ...
                      $insert_device = "INSERT INTO REPAIR (REPAIRID, DEV_ID, USERNAME)
                                      VALUES ( '$repid','$device_id', '$usrname')";
                      $servicecheck = "SELECT * FROM REPAIR WHERE (REPAIRID = '$repid' AND DEV_ID = '$device_id')";
                      $result = mysqli_query($con, $servicecheck);
                        if($result->num_rows > 0) {
                          $row = mysqli_fetch_assoc($result);
                          $_SESSION['REPAIRID'] = $row['REPAIRID'];
                          $_SESSION['DEV_ID'] = $row['DEV_ID'];
                          $message="Repair ID or Device ID is already exist. Please try again.";
              ...
              <div class="devicerepair_container">
                ...
                if ($result = $con->query($query)) {
                  while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<textarea type="text" rows="3" cols="15" style="text-align:center; font-size:15px">Your payment will be RM '.$repcharge.'</textarea><br>';
                  }
                  $result->free();
                }
                  ?>
